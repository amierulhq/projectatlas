...
<div class="row">
	...
	<div class="table-responsive">
		...
		?>
                <tr>
					<th>No.</th>
                    <th><?= $this->Paginator->sort('name', ['label' => 'Full Name']) ?></th>
				...
				<td><?php echo $counter++ ?></td>
					...
					<td>
						...
						if ($lecturer->status == 1){
							...
							echo '<span class="badge text-bg-warning">On Leave</span>';
						?>
					</td>
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $lecturer->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $lecturer->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Lecturers: "{0}"?', $lecturer->id),
									'title' => __('Delete'),
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Lecturers(s)',
			data: [<?= json_encode($total_lecturers_active); ?>, <?= json_encode($total_lecturers_disabled); ?>, <?= json_encode($total_lecturers_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Filter By</div>
			<div class="tricolor_line mb-3"></div>
				<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Lecturers','action' => 'index']]); ?>
					<fieldset>
						<div class="mb-1">
							<?= $this->Form->control('name', ['label' => 'Full Name']) ?>
						</div>
						<div class="mb-1"><?php echo $this->Form->control('staff_no',['required' => false]); ?></div>
						<div class="mb-1">
