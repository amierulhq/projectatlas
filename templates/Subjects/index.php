	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('credit_hour') ?></th>
				...
				<td><?php echo $counter++ ?></td>
					...
					<td>
						...
						if ($subject->status == 1){
							...
							echo '<span class="badge text-bg-warning">Under Development</span>';
						?>
					</td>
                    <td><?= h($subject->credit_hour) ?></td>
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $subject->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $subject->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Subjects: "{0}"?', $subject->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_subjects_active; ?></h3>
					<p>Active Subjects</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_subjects_archived; ?></h3>
					<p>Archived Subjects</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Subjects (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Subjects by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Subjects(s)',
			data: [<?= json_encode($total_subjects_active); ?>, <?= json_encode($total_subjects_disabled); ?>, <?= json_encode($total_subjects_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Filter By</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Subjects','action' => 'index']]); ?>
				<fieldset>
